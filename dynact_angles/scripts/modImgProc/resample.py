...
-------
"""
def resample(input_path, output_path, new_spacing, new_origin, new_size, 
    ...
    if interpolator is None or interpolator.lower() not in interp_dict:
        ...
        print('** Using: provided interpolator: ' + str(interp_dict_enum.get(sitk_interp)))

    if new_origin is None:
        print('** Invalid or no image origin provided. Using: ' + str(image_origin) + ' as the resampled origin')
        resampled_origin = image_origin
    else:
        resampled_origin = new_origin
        print('** Using provided origin: ' + str(resampled_origin) + ' as the resampled origin')

    if new_direction is None:
        print('** Invalid or no image direction provided. Using: ' + str(image_direction) + ' as the resampled direction')
        resampled_direction = image_direction
    else:
        resampled_direction = new_direction
        print('** Using provided direction: ' + str(resampled_direction) + ' as the resampled direction')

    if new_size is None:
        print('** Invalid or no image dimensions provided. Calculating new image size...')

        # Formula to calculate new image dimensions:
        # newDim = oldDim * oldSpacing / new_spacing
        temp = [image_spacing[0] / resampled_spacing[0],
                image_spacing[1] / resampled_spacing[1],
                image_spacing[2] / resampled_spacing[2]]
        temp = [int(temp[0] * image_size[0]),
    ...
    if output_data_type is None or output_data_type not in sitk_pixelID_enum:
        print('** Invalid or no output data type provided. Using: ' + str(image.GetPixelIDTypeAsString()))
        resampled_data_type = image_data_type
    else:
        resampled_data_type = output_data_type
        print('** Using provided data type: ' + str(sitk_pixelID_enum.get(resampled_data_type)))
    ...
    resample_filter.AddCommand(sitk.sitkProgressEvent, lambda: print("\rProgress: {0:03.1f}%...".format(100*resample_filter.GetProgress()),end=''))
...
if __name__ == "__main__":
    ...
    resample(input_pathAbs, outputImage, new_spacing, new_origin, new_size, \
                new_direction, interpolator, output_data_type)
