...
def initialize_registration(fixed_landmarks, moving_landmarks):
	...
	ref_transform = sitk.LandmarkBasedTransformInitializer(sitk.Similarity3DTransform(), 
...
if __name__ == "__main__":
	...
	parser.add_argument('fixed_img_path', type=str, \
						help='The fixed image (path + filename)')
	parser.add_argument('fixed_landmarks_path', type=str, \
						help='The fixed landmarks (segmentation)  image (path + filename)')
	parser.add_argument('moving_img_path', type=str, \
						help='The moving image (path + filename)')
	parser.add_argument('moving_landmarks_path', type=str, \
						help='The moving landmarks (segmentation)  image (path + filename)')
	parser.add_argument('output_path', nargs='?', type=str, \
						help='The directory for any outputs', default=os.getcwd())
	...
	moving_img_registered_path = os.path.join(output_path, scan + '_' + bone + '_REG.nii')
	moving_img_tmat_path = os.path.join(output_path, scan + '_' + bone + '_REG.tfm')
	
	moving_resampled = main(fixed_landmarks_path, moving_landmarks_path, \
