...
def main(fixed_img_path, moving_img_path, tfm_path, inverse_tfm, interpolator):
    ...
    print('RESAMPLING: ')
    print(f'{moving_img_path}')
    resampled = sitk.Resample(moving, fixed, tmat, interpolator, 0.0, moving.GetPixelID())
...
if __name__ == "__main__":
    ...
    parser.add_argument('moving_img', type=str, help='Moving image to be transformed (path + filename)')
    parser.add_argument('tfm', type=str, help='The transformation matrix .TFM file (path + filename)')
    parser.add_argument('-o', dest='output_file', type=str, default='', \
                        help='Output image (path + filename). Default = moving image directory')
    parser.add_argument('-i', dest='interpolator', type=str, default='linear', \
                        help='SimpleITK interpolator type (nn, linear, spline, or gaussian). Default = linear')
    parser.add_argument('-r', dest='inverse_tfm', default=False, \
    ...
    if not output_file :
        ...
        print()
        print('Warning: No output file name or directory provided. Writing transformed image to: ')
        print(f'{output_file}')
