...
def main(rater, next_scan):
    ...
    ct2xct_reg_dir = os.path.join(reg_dir, os.path.join(next_scan, 'staticCT_to_HR-pQCT'))
    ...
    ct2dynact_reg_dir = os.path.join(reg_dir, os.path.join(next_scan, 'staticCT_to_dynamicCT/B'))

    dynact_pnts_dir = os.path.join(point_dir, os.path.join(next_scan, 'dynamicCT'))
    dynact_reg_dir = os.path.join(reg_dir, os.path.join(next_scan, 'dynamicCT_frames/B/FinalTFMs'))

    if '3' in next_scan or '5' in next_scan or '7' in next_scan or '9' in next_scan:
        ct2dynact_reg_dir = os.path.join(reg_dir, os.path.join(next_scan, 'staticCT_to_dynamicCT/C'))
        dynact_reg_dir = os.path.join(reg_dir, os.path.join(next_scan, 'dynamicCT_frames/C/FinalTFMs'))
    ...
    mc1_pnts_file = os.path.join(point_dir, str(next_scan) + 'e_MC1_SCS_' + str(rater) + '.txt')
    trp_pnts_file = os.path.join(point_dir, str(next_scan) + 'e_TRP_SCS_' + str(rater) + '.txt')
    
    mc1_pnts_list = [None] * 3
    trp_pnts_list = [None] * 4

    print(Colours.BLUE + '\t Reading in MC1 points: ' + Colours.WHITE + '{}'.format(mc1_pnts_file))
    try:
    ...
    print(Colours.BLUE + '\t Reading in TRP points: ' + Colours.WHITE + '{}'.format(trp_pnts_file))
    try:
    ...
    #-------------------------------------------------------#
    arr = xct_to_dynact_transform(xct_dir_list, ct_dir_list, dynact_dir_list, mc1_pnts_list, trp_pnts_list)
...
Rxyz = Rz * Ry * Rx
Rxyz = [ cos(b)cos(g)   (sin(a)sin(b)cos(g) + cos(a)sin(g))   (-cos(a)sin(b)cos(g) + sin(a)sin(g)) ]
...
def compute_translations(R_relative, curr_origin, prev_origin):
    transformed_origin =  R_relative.dot(np.array([float(curr_origin[0]), \
                                                    float(curr_origin[1]), \
                                                    float(curr_origin[2])]))
...
"""
def dynact_frame_transform(dynact_reg_dir, mc1_pnts_dynact, trp_pnts_dynact, R_relative):
    ...
    transformed_origin = R_relative.dot(np.array([float(mc1_pnt3_dynact[0]), \
    ...
    for i in range(2,60):
        ...
        # MC1
        dynact_reg_path_mc1 = os.path.join(dynact_reg_dir, 'VOLUME_REF_TO_' + str(i) + '_MC1_REG.tfm')
        dynact_next_reg_mc1_tfm = sitk.ReadTransform(dynact_reg_path_mc1)
        
        mc1_pnt1_dynact_frame = transform_point(dynact_next_reg_mc1_tfm, mc1_pnt1_dynact)
        mc1_pnt2_dynact_frame = transform_point(dynact_next_reg_mc1_tfm, mc1_pnt2_dynact)
        mc1_pnt3_dynact_frame = transform_point(dynact_next_reg_mc1_tfm, mc1_pnt3_dynact)

        # TRP
        dynact_reg_path_trp = os.path.join(dynact_reg_dir, 'VOLUME_REF_TO_' + str(i) + '_TRP_REG.tfm')
        dynact_next_reg_trp_tfm = sitk.ReadTransform(dynact_reg_path_trp)

        trp_pnt1_dynact_frame = transform_point(dynact_next_reg_trp_tfm, trp_pnt1_dynact)
        trp_pnt2_dynact_frame = transform_point(dynact_next_reg_trp_tfm, trp_pnt2_dynact)
        trp_pnt3_dynact_frame = transform_point(dynact_next_reg_trp_tfm, trp_pnt3_dynact)
        trp_pnt4_dynact_frame = transform_point(dynact_next_reg_trp_tfm, trp_pnt4_dynact)
        ...
        # [Yx  Yy  Yz]
        # [Zx  Zy  Zz]
        xct_mc1_scs = np.array(calculate_mc1_scs([mc1_pnt1_dynact_frame, mc1_pnt2_dynact_frame, mc1_pnt3_dynact_frame]))
        xct_trp_scs = np.array(calculate_trp_scs([trp_pnt1_dynact_frame, trp_pnt2_dynact_frame, trp_pnt3_dynact_frame, trp_pnt4_dynact_frame]))
...
arr : list
"""
def xct_to_dynact_transform(xct_dir_list, ct_dir_list, dynact_dir_list, mc1_pnts_list, trp_pnts_list):
    #----------------------------------------------------------#
    ...
    # [Yx  Yy  Yz]
    # [Zx  Zy  Zz]
    xct_mc1_scs = np.array(calculate_mc1_scs([mc1_pnt1_dynact, mc1_pnt2_dynact, mc1_pnt3_dynact]))
    xct_trp_scs = np.array(calculate_trp_scs([trp_pnt1_dynact, trp_pnt2_dynact, trp_pnt3_dynact, trp_pnt4_dynact]))
    ...
    if debug:
        ...
        print(Colours.PURPLE + '\t DEBUG: XCT relative rotation matrix:' + Colours.WHITE)
        print('\t R_relative: \n\t ' + str(R_relative).replace('\n','\n \t'))
        print()
        
    mc1_pnts_dynact = [mc1_pnt1_dynact, mc1_pnt2_dynact, mc1_pnt3_dynact]
    trp_pnts_dynact = [trp_pnt1_dynact, trp_pnt2_dynact, trp_pnt3_dynact, trp_pnt4_dynact]
    rotations, translations = dynact_frame_transform(dynact_reg_dir, mc1_pnts_dynact, trp_pnts_dynact, R_relative)
...
if __name__ == "__main__":
    ...
    rater_list = ['JJT', 'TB', 'MTK001', 'MTK002', 'MTK003']
    header_arr = np.array([['Frame', \
                            'JJT_AbAd', 'JJT_FlexExt', 'JJT_Rot', \
                            'JJT_X', 'JJT_Y', 'JJT_Z', \
                            'TB_AbAd', 'TB_FlexExt', 'TB_Rot', \
                            'TB_X', 'TB_Y', 'TB_Z', \
                            'MTK_001_AbAd', 'MTK_001_FlexExt', 'MTK_001_Rot', \
                            'MTK_001_X', 'MTK_001_Y', 'MTK_001_Z', \
                            'MTK_002_AbAd', 'MTK_002_FlexExt', 'MTK_002_Rot', \
                            'MTK_002_X', 'MTK_002_Y', 'MTK_002_Z', \
                            'MTK_003_AbAd', 'MTK_003_FlexExt', 'MTK_003_Rot', \
                            'MTK_003_X', 'MTK_003_Y', 'MTK_003_Z']], dtype=object)
    ...
    for sub_dir in next(os.walk(model_dir))[1]:
        ...
        print(Colours.BOLD + 'Computing angles for ' + str(sub_dir) + '...' + Colours.WHITE)

        output_arr = frame_arr
        output_arr = np.hstack([output_arr, main(rater_list[0], sub_dir)])
        output_arr = np.hstack([output_arr, main(rater_list[1], sub_dir)])
        output_arr = np.hstack([output_arr, main(rater_list[2], sub_dir)])
        output_arr = np.hstack([output_arr, main(rater_list[3], sub_dir)])
        output_arr = np.hstack([output_arr, main(rater_list[4], sub_dir)])
