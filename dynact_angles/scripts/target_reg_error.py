...
def get_landmarks(xct_landmark_img, ct_landmarks_img, fiducial_list):
    ...
    xct_target1 = np.array([xct_landmarks_flat[0], xct_landmarks_flat[1], xct_landmarks_flat[2]])
    xct_target2 = np.array([xct_landmarks_flat[3], xct_landmarks_flat[4], xct_landmarks_flat[5]])
    xct_target3 = np.array([xct_landmarks_flat[6], xct_landmarks_flat[7], xct_landmarks_flat[8]])
    ...
    ct_target1 = np.array([ct_landmarks_flat[0], ct_landmarks_flat[1], ct_landmarks_flat[2]])
    ct_target2 = np.array([ct_landmarks_flat[3], ct_landmarks_flat[4], ct_landmarks_flat[5]])
    ct_target3 = np.array([ct_landmarks_flat[6], ct_landmarks_flat[7], ct_landmarks_flat[8]])
    ...
    xct_fiducials = [[xct_F1_centroid[0], xct_F1_centroid[1], xct_F1_centroid[2]], \
    ...
    ct_fiducials = [[ct_F1_centroid[0], ct_F1_centroid[1], ct_F1_centroid[2]], \
...
def compute_TGS(xct_fiducials_flat, ct_fiducials_flat):
    ...
    sitk.WriteTransform(Tgs, '/Users/mkuczyns/OneDrive - University of Calgary/ManskeLabImages/DYNACT/04 - DYNACT1/reg/DYNACT1_011/staticCT_to_HR-pQCT/TGS.tfm')
...
def compute_FRE(T_gs, xct_centroids, ct_centroids):
    ...
    fre1 = math.sqrt((1/Nf) * ((ct_F1_centroid[0] - xct_F1_centroid[0]) ** 2) + 
                                ((ct_F1_centroid[1] - xct_F1_centroid[1]) ** 2) + 
                                ((ct_F1_centroid[2] - xct_F1_centroid[2]) ** 2))
    fre2 = math.sqrt((1/Nf) * ((ct_F2_centroid[0] - xct_F2_centroid[0]) ** 2) + 
                                ((ct_F2_centroid[1] - xct_F2_centroid[1]) ** 2) + 
                                ((ct_F2_centroid[2] - xct_F2_centroid[2]) ** 2))
    fre3 = math.sqrt((1/Nf) * ((ct_F3_centroid[0] - xct_F3_centroid[0]) ** 2) + 
...
def compute_TRE(Nf, dx, dy, dz, rms_fx, rms_fy, rms_fz, fle):
    tre = math.sqrt((1/Nf) * (1 + (1/3) * ((dx**2)/(rms_fx**2) + \
                    (dy**2)/(rms_fy**2) + (dz**2)/(rms_fz**2))) * fle**2)
    return tre
...
def compute_PA(combined_img):
    ...
    PA_x = np.array([fiducial_img_PA(0,0), fiducial_img_PA(0,1), fiducial_img_PA(0,2)])
    PA_y = np.array([fiducial_img_PA(1,0), fiducial_img_PA(1,1), fiducial_img_PA(1,2)])
    PA_z = np.array([fiducial_img_PA(2,0), fiducial_img_PA(2,1), fiducial_img_PA(2,2)])
...
def main(image_list, fiducial_list, landmark_list, output_TGS, intensity_TFM):
    ...
    fiducial_img_list = [xct_F1_img, xct_F2_img, xct_F3_img, ct_F1_img, ct_F2_img, ct_F3_img]

    xct_fiducials_flat, ct_fiducials_flat, xct_targets, ct_targets = get_landmarks(xct_landmark_img, \
                                                                                    ct_landmarks_img, \
                                                                                    fiducial_img_list)
    ...
    ct_transformed_Tgs = sitk.Resample(ct_img, xct_img, T_gs, sitk.sitkLinear, 0.0, ct_img.GetPixelID())
    ...
    errors = [ (np.linalg.norm(np.array(p_fixed) -  np.array(p_moving)))
            for p_fixed, p_moving in zip(transformed_ct_landmarks_Tgs, transformed_ct_landmarks_Tr)]
    min_errors = np.min(errors)
    max_errors = np.max(errors)
    print('TRE between gold-standard and intensity-based registration for landmarks = ' + str(errors))
    ...
    d, xct_F1_d, xct_F2_d, xct_F3_d = get_distance_to_PA(T_gs, xct_targets, \
                                                        PA_xct, \
